...
const BoardDetail = () => {
  ...
  const upLike=async()=>{
    try {
      await likeBoard({variables: {boardId: router.query.boardId},
    ...
    const downlike=async()=>{
      try {
        await dislikeBoard({variables: {boardId: router.query.boardId},
          refetchQueries: [{query: FETCH_BOARD, variables: {boardId: router.query.boardId}}]
        })
      } catch(error: any) {
        alert(error.message)
  ...
  return (
    <BoardDetailUI upLike={upLike} downlike={downlike} handleEdit={handleEdit} date = {date} addressShow = {handleClickShow} data = {data} handleDeleteBoard={handleDeleteBoard} handleList={handleList}/>
  )
